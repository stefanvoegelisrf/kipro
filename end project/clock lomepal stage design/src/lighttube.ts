...
export class LightTube {
    ...
    display(sketch: p5) {
        ...
        let glowBorder = this.glowMaxRadius * 0.5; // Border of the glow
        let steps = this.glowMaxRadius * 0.5; // Number of steps for the glow effect
        sketch.noFill();

        for (let i = 0; i < steps; i++) {
            let alpha = sketch.map(i, 0, steps, this.maxAlpha, 0); // Decreasing alpha value
            let spread = sketch.map(i, 0, steps, 0, this.glowMaxRadius); // Increasing spread of the glow
